...
static const struct option thread_options[] = {
	OPT_UINTEGER('b', "breakpoints", &thread_params.nbreakpoints, "Specify amount of breakpoints"),
...
int bench_breakpoint_thread(int argc, const char **argv)
{
	...
	case BENCH_FORMAT_DEFAULT:
		...
		printf(" %14lf usecs/op/cpu\n",
		       (double)result_usec / bench_repeat / thread_params.nthreads * thread_params.nparallel);
		break;
...
{
	unsigned i, nthreads, result_usec, done = 0;
	char watched;
	int fd;
	pthread_t *threads;
	struct timeval start, stop, diff;
